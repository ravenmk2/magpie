...
import ravenworks.magpie.testkit.probe.ProbeMessage;
...
public class VerifyController {
    ...
    public VerifyController(Map<String, SequenceTracker> trackers, MeterRegistry metrics) {
        ...
        trackers.keySet().forEach(channel -> {
            this.latencyTimers.put(channel,
                    Timer.builder("testkit.e2e.latency")
                            .description("probe end-to-end latency")
                            .tag("channel", channel)
                            .publishPercentileHistogram(true)
                            .register(metrics));
            // 同样启动即注册：零值序列让"无无效探针"与"没采集到"可区分
            this.invalidCounters.put(channel, metrics.counter("testkit.invalid", "channel", channel));
        });
    }
    ...
    private void invalid(String channel, String reason) {
        ...
        if (counter != null) {
            ...
            this.metrics.counter("testkit.invalid", "channel", channel).increment();
        }
