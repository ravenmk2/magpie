...
public class Publisher {
    ...
    public Publisher(List<String> endpoints, String source, Duration timeout,
        ...
        this.topicCounters = topics.stream().collect(Collectors.toMap(Function.identity(),
                topic -> new Counters(
                        metrics.counter("testkit.sent", "topic", topic),
                        metrics.counter("testkit.send.errors", "topic", topic),
                        metrics.counter("testkit.send.rejected", "topic", topic))));
    }
    ...
    public boolean send(String topic, byte[] body) {
        ...
        for (int i = 0; i < this.publishUrls.size(); i++) {
            ...
            try {
                ...
                log.error("publish rejected with HTTP {}, check testkit seed config", status);
                return false;
            } catch (IOException e) {
                counters.errors.increment();
