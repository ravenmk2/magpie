...
#
# 用法：
#   deploy/soak-test/fault-inject.sh kill-magpie        # docker kill（SIGKILL）随机一台 magpie
#   deploy/soak-test/fault-inject.sh restart-rabbit     # 滚动重启随机一个 RabbitMQ 节点
#   deploy/soak-test/fault-inject.sh bounce-mysql       # docker kill MySQL（短暂停机后脚本拉起）
#   deploy/soak-test/fault-inject.sh partition-rabbit [秒]  # 随机 RabbitMQ 节点断网 N 秒（默认 30）后恢复
#   deploy/soak-test/fault-inject.sh random             # 随机执行上述一种
#   deploy/soak-test/fault-inject.sh loop [最小间隔秒] [最大间隔秒]  # 循环随机注入（默认 300~900s）
#
...
MAGPIES=(magpie-soak-magpie-1 magpie-soak-magpie-2)
RABBITS=(magpie-soak-rabbitmq-1 magpie-soak-rabbitmq-2 magpie-soak-rabbitmq-3)
NETWORK=magpie-soak
MYSQL=magpie-soak-mysql
