...
```bash
scripts/soak-test-up.sh                # 可加 -DskipTests 跳过构建期单测；--skip-build 跳过构建
```
...
scripts/build-testkit-image.sh

# 2. 配置
cp deploy/soak-test/.env.example deploy/soak-test/.env   # 默认值即可起步

# 3. 启动
docker compose -f deploy/soak-test/compose.yml up -d

# 4. 确认
docker compose -f deploy/soak-test/compose.yml ps
docker logs -f magpie-soak-verifier                    # 等 "verify report" 周期性输出
```

首次启动时 rabbitmq-2/3 会等 rabbitmq-1 就绪（先形成单节点集群）后再加入，比后续启动慢十几秒属预期——
virgin 节点同时首启存在组网竞态，compose 里已用 `depends_on: service_healthy` 定序规避；若个别容器
仍首启失败，重跑一次 `up -d` 即可（卷与集群身份已建立，第二次启动没有竞态窗口）。
...
- 关键指标：`testkit_lost_total` / `testkit_out_of_order_total`（语义违规，恒应为 0）、
  `testkit_duplicates_total`（at-least-once 的正常重复）、`testkit_e2e_latency_seconds`（端到端延迟）
...
```bash
deploy/soak-test/fault-inject.sh kill-magpie         # SIGKILL 随机一台 magpie，短暂停机后拉起
deploy/soak-test/fault-inject.sh restart-rabbit      # 滚动重启随机 RabbitMQ 节点
deploy/soak-test/fault-inject.sh bounce-mysql        # SIGKILL MySQL，短暂停机后拉起
deploy/soak-test/fault-inject.sh partition-rabbit 60 # 随机 RabbitMQ 节点断网 60s
deploy/soak-test/fault-inject.sh loop                # 循环随机注入（默认间隔 300~900s）
```
...
```bash
# 方式一：命令行启用
docker compose -f deploy/soak-test/compose.yml --profile chaos up -d
# 方式二：.env 里加 COMPOSE_PROFILES=chaos，之后 up -d 即自动带上
```

- 注入间隔由 `CHAOS_MIN_INTERVAL` / `CHAOS_MAX_INTERVAL` 控制（秒，区间内随机，默认 300~900）
- 注入时间线：`docker logs -f magpie-soak-chaos`，与 Grafana 看板对照即可把语义违规归因到具体故障
...
- **前置**：应用探针依赖镜像内的 curl，更新 compose 后必须重新执行
  `scripts/build-image.sh` / `scripts/build-testkit-image.sh`，否则探针永远失败、
  autoheal 会陷入重启循环
...
改后 `docker compose -f deploy/soak-test/compose.yml up -d` 重建 loadgen 生效，无需重新构建镜像。
...
```bash
docker compose -f deploy/soak-test/compose.yml down -v   # -v 清卷：MySQL 重跑 schema+seed，stream 重建
docker compose -f deploy/soak-test/compose.yml up -d
```
