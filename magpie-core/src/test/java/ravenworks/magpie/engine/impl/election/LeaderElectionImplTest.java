...
class LeaderElectionImplTest {
    ...
    static class FakeLockRepository {
        ...
        volatile int acquireFailures;
        volatile int renewFailures;
        volatile int renewRows = 1;
        volatile int saveFailures;
                    ...
                    (proxy, method, args) -> switch (method.getName()) {
                        ...
                        case "save" -> {
                            ...
                            if (this.saveFailures > 0) {
                                this.saveFailures--;
                                throw new DataIntegrityViolationException("duplicate key (simulated)");
                            }
    ...
    @Test
    void acquireExceptionResetsSilentlyAndSelfHeals() {
        var repo = new FakeLockRepository();
        repo.acquireFailures = 1;
        var election = new LeaderElectionImpl(repo.proxy(), HEARTBEAT_MS);
        var events = new CopyOnWriteArrayList<LeaderElection.Event>();
        election.addListener(events::add);
        try {
            election.start();
            // 首拍 acquireLock 抛异常：tick 走 catch 分支复位 setLeader(false)，
            // 但本就未持有锁，compareAndSet 不命中，不得派发 LOST 事件。
            // 下拍自愈：acquireLock 正常，锁行不存在则插入首行并持有
            await().atMost(2, TimeUnit.SECONDS).until(election::isLeader);
            assertEquals(InstanceId.VALUE, repo.lock.getInstanceId());
            assertTrue(repo.acquireCalls.get() >= 2, "失败后应由后续节拍重新抢占");
            // 全程无虚假 LOST：事件序列只有 ACQUIRED
            assertEquals(List.of(LeaderElection.Event.ACQUIRED), events);
        } finally {
            election.shutdown().join();
        }
    ...
    @Test
    void saveConflictOnFirstRowInsertFailsThisTickAndSelfHeals() {
        var repo = new FakeLockRepository();
        repo.saveFailures = 1;
        var election = new LeaderElectionImpl(repo.proxy(), HEARTBEAT_MS);
        var events = new CopyOnWriteArrayList<LeaderElection.Event>();
        ...
        try {
            election.start();
            // 首行插入 PK 冲突（并发实例抢先插入）：本轮 acquire 返回 false，不得持有锁。
            // 下个心跳重新尝试插入自愈（此处锁行被 fake 吞掉，故重插成功）
            await().atMost(2, TimeUnit.SECONDS).until(election::isLeader);
            assertEquals(InstanceId.VALUE, repo.lock.getInstanceId());
            assertEquals(List.of(LeaderElection.Event.ACQUIRED), events);
            // 恰好两次 save：第一次冲突失败，第二次插入成功
            assertEquals(2, repo.saveCalls.get());
        } finally {
            election.shutdown().join();
        }
    }
    ...
    @Test
    void failingListenerIsIsolatedFromRemainingListeners() {
        var repo = new FakeLockRepository();
        var election = new LeaderElectionImpl(repo.proxy(), HEARTBEAT_MS);
        var events = new CopyOnWriteArrayList<LeaderElection.Event>();
        // 先注册一个抛异常的监听器：setLeader 逐个 try/catch，后续监听器仍须收到事件
        election.addListener(event -> {
            throw new RuntimeException("listener failed (simulated)");
        });
        election.addListener(events::add);
        try {
            election.start();
    ...
    @Test
    void releaseFailureDuringShutdownDoesNotHangNorDispatchLost() {
        var repo = new FakeLockRepository();
        var election = new LeaderElectionImpl(repo.proxy(), HEARTBEAT_MS);
        var events = new CopyOnWriteArrayList<LeaderElection.Event>();
        election.addListener(events::add);
        ...
        // 放锁抛异常（如 DB 瞬断）：仅记日志、内部状态复位，
        // 关停不挂死、不派发 LOST（停机流程由调用方主导）
        repo.releaseFailures = 1;
        election.shutdown().join();
