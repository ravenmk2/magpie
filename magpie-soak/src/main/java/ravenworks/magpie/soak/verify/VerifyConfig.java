...
public class VerifyConfig {
    ...
    public Map<String, SequenceTracker> trackers(VerifyProperties props, MeterRegistry metrics) {
        ...
        props.getChannels().forEach((channel, semantics) -> {
            ...
            metrics.gauge("soak.active_keys", Tags.of("channel", channel),
    ...
    private static final class ChannelListener implements SequenceTracker.Listener {
        ...
        private ChannelListener(String channel, MeterRegistry metrics) {
            this.received = metrics.counter("soak.received", "channel", channel);
            this.duplicates = metrics.counter("soak.duplicates", "channel", channel);
            this.outOfOrder = metrics.counter("soak.out_of_order", "channel", channel);
            this.lost = metrics.counter("soak.lost", "channel", channel);
        }
